...
Beyond the above: `check_output_conformance` checked a `<q:send>`'s render against the
...
The catalog generator writes pages but never prunes, so a renamed source leaves an orphan
that `--check` cannot see. No operations console UI. No published performance numbers.
