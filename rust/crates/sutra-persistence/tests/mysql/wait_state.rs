//! Mirrors `tests/pg/wait_state.rs` (wait-point registry) on the MySQL dialect.

use sutra_persistence::mysql::stores::MySqlWaitStateStore;
use sutra_persistence::stores::{WaitStateStore, WaitingFilter, STATUS_RESOLVED, STATUS_WAITING};
use uuid::Uuid;
...
async fn record_is_idempotent() {
    ...
    // A redelivery re-parks the same token: back to WAITING, no duplicate, resolved_at
    // cleared.
    store
        .record_waiting(&dep_a(), instance, "loan", "waitA", None)
        .await
