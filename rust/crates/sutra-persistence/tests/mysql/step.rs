...
//! commit-or-nothing proofs on the MySQL dialect (crash injection included).

use std::collections::BTreeMap;

use sutra_persistence::mysql::step::{commit_step, write_step_in};
use sutra_persistence::mysql::stores::{
...
async fn table_counts(pool: &sqlx::MySqlPool) -> (i64, i64, i64, i64) {
...
async fn step_commits_snapshot_waits_aliases_and_outbox_atomically() {
    ...
    let instance_store = MySqlInstanceStore::new(pool.clone());
    ...
    let wait_store = MySqlWaitStateStore::new(pool.clone());
    ...
    let alias_store = MySqlAliasStore::new(pool.clone());
    assert_eq!(
        alias_store
    ...
    let outbox_store = MySqlOutboxStore::new(pool.clone());
...
async fn induced_failure_before_commit_leaves_zero_rows_in_all_tables() {
    ...
    // Crash injection: perform every step write, then abandon the transaction (rollback
    // on drop) — the process dying between write and commit.
    {
...
async fn wait_to_wait_step_resolves_prior_frontier_and_writes_next() {
    ...
    let instance_store = MySqlInstanceStore::new(pool);
