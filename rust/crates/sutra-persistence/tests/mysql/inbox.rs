//! Mirrors `tests/pg/inbox.rs` (first-observer inbox dedup) on the MySQL dialect.

use std::time::Duration;

use sutra_persistence::mysql::stores::MySqlInboxStore;
...
async fn insert_with_seen_at(
    ...
    let micros = (age_secs * 1_000_000.0) as i64;
    sqlx::query(
        "INSERT INTO inbox_seen (deployment_id, channel, event_id, seen_at) \
         VALUES (?, ?, ?, TIMESTAMPADD(MICROSECOND, -?, NOW(6)))",
    )
    .bind(dep.as_str())
    .bind(channel)
    .bind(event_id)
...
async fn prune_removes_old_rows_across_deployments() {
    let pool = fresh_pool().await;
    let store = MySqlInboxStore::new(pool.clone());
