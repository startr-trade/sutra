//! Mirrors `tests/pg/alias.rs` (unique-LIVE alias guarantee) on the MySQL
//! dialect — here the guarantee rides the generated-column unique key of the dialect's
//! V101 instead of a partial index; every observable behaviour must be identical.

use sutra_persistence::mysql::stores::MySqlAliasStore;
...
async fn retire_makes_alias_unusable() {
    ...
    // The unique-live key no longer covers the retired row (its generated marker went
    // NULL): a NEW instance can claim the same alias.
