//! Deployment-archive store semantics (V1001) on the MySQL dialect — mirrors the reference
//! `PgDeploymentArchiveStore` behaviour: a first deploy stores an active row, a hot-deploy
//! replaces the slot in place (prior active → draining, bumped per-slot revision, exactly one
//! active row per slot), a same-id rollback re-activates in place, distinct slots coexist
//! active, and get_bytes/retire_slot behave.

use sutra_persistence::mysql::stores::MySqlDeploymentArchiveStore;
...
async fn first_deploy_stores_active_row() {
    let store = MySqlDeploymentArchiveStore::new(fresh_pool().await);
    let rev = store
