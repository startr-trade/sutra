...
use sutra_persistence::mssql::stores::MssqlLeaseStore;
...
async fn backdate_expiry(pool: &MssqlPool, name: &str, secs: f64) {
    let millis = (secs * 1000.0) as i32;
    let mut conn = pool.acquire().await.unwrap();
    conn.client()
        .execute(
            "UPDATE lease SET expires_at = DATEADD(MILLISECOND, -@P1, SYSUTCDATETIME()) \
             WHERE name = @P2",
            &[&millis, &name],
        )
        .await
...
async fn acquire_fresh() {
    ...
    let store = MssqlLeaseStore::new(pool);
...
async fn acquire_after_expiry() {
    let pool = fresh_pool().await;
    let store = MssqlLeaseStore::new(pool.clone());
...
async fn concurrent_acquire_single_winner() {
    ...
    assert_eq!(
        winners, 1,
        "the range-locked acquire admits exactly one holder"
    );
}
