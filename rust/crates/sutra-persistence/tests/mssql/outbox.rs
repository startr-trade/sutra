//! Mirrors `tests/pg/outbox.rs` (outbox claim/defer/delete + the READPAST
//! concurrent-claim proof — the SQL Server SKIP LOCKED equivalent).

use std::collections::BTreeMap;

use sutra_persistence::mssql::stores::MssqlOutboxStore;
...
async fn enqueue_and_claim() {
    ...
    let store = MssqlOutboxStore::new(pool);
...
async fn concurrent_claims_skip_locked_rows() {
    let pool = fresh_pool().await;
    let store = MssqlOutboxStore::new(pool.clone());
    for _ in 0..4 {
        store.enqueue(&entry(Uuid::new_v4())).await.unwrap();
    }
    ...
    let first = MssqlOutboxStore::claim_due_in(tx1.client(), &dep_a(), now_micros(), 2)
        .await
        .unwrap();
    assert_eq!(first.len(), 2);

    // Replica 2, concurrently, must READPAST the locked rows and claim the OTHER two —
    // never blocking, never double-claiming.
    let mut tx2 = MssqlTx::begin(&pool).await.unwrap();
    let second = MssqlOutboxStore::claim_due_in(tx2.client(), &dep_a(), now_micros(), 10)
        .await
        .unwrap();
    ...
    for e in &second {
        assert!(
            ...
            "READPAST must yield disjoint claims"
        );
    }
