//! `PgChannelConcurrencyStore` against a real PostgreSQL: the started/waiting/resumed/terminal
//! transitions and what each contributes to `count_active_by_channel`, the `include_waiting`
//! flag, per-channel and per-deployment isolation, idempotent starts, and unknown-instance
//! transitions being no-ops.

use sutra_persistence::stores::{ChannelConcurrencyStore, PgChannelConcurrencyStore};
use uuid::Uuid;
...
async fn started_then_terminal() {
    ...
    let store = PgChannelConcurrencyStore::new(pool);
