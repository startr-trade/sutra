//! TIMER wait rows (this engine only — the reference baseline has no timers): the V803
//! `waiting_event` addendum, the SKIP-LOCKED due-claim, defer backoff, and the
//! park-with-timer step primitive's commit-or-nothing behavior.
...
use sutra_persistence::step::{
    commit_step_with_timers, write_step_with_timers_in, StepTimerWait, StepWait, StepWrite,
};
use sutra_persistence::stores::{PgWaitStateStore, WaitStateStore, WaitingFilter, STATUS_RESOLVED};
use time::Duration;
use uuid::Uuid;
...
async fn concurrent_claims_never_hand_out_the_same_timer() {
    ...
    let mut held = sqlx::PgConnection::connect(&db_url(&db)).await.unwrap();
    ...
    sqlx::query("SELECT set_config('sutra.deployment_id', $1, true)")
        ...
        .await
        .unwrap();
    let first = PgWaitStateStore::claim_due_timers_in(&mut tx, &dep_a(), now_micros(), 10)
        .await
        .unwrap();
